...
# ---------------- GLOBALS ----------------
class_num = 14 # 0 or 14 for person or bird respectively
state = 0
last_state = -1
alarm_active = False
...
# ---------------- STREAM ----------------
def generate(cam_id):
    global latest_frame1, latest_frame2, recording_enabled
...
def boxes():
    if latest_state == 0:
        state_msg = "No bird detected"
    elif latest_state == 1:
        state_msg = "Bird sighted!"
    elif latest_state == 2 or latest_state == 3:
        state_msg = "Bird is closer than before!"
    return jsonify({
        "cam1": latest_boxes1,
        "cam2": latest_boxes2,
        "state": latest_state,
...
def index():
    return """
<html>
<body style="margin:0;background:#2f6969;color:white;">
<h2 id="status"></h2>
...
async function toggle() {
    if (recording) {
        ...
        // restart stream
        document.getElementById("cam1").src = "/cam1?" + Date.now();
        document.getElementById("cam2").src = "/cam2?" + Date.now();
    }
...
<button onclick="toggle()" id="toggleBtn">Toggle Stream/Recording</button>
...
class App:
    ...
    def loop(self):
        ...
        while True:
            ...
            framefull1 = cv.rotate(framefull1, cv.ROTATE_90_COUNTERCLOCKWISE)
            framefull2 = cv.rotate(framefull2, cv.ROTATE_90_CLOCKWISE)
